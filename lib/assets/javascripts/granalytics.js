...
var TimeLineOptions = function($el, response) {
  ...
  this.lang.noData = subWithResponse($el.data('no-data-text'), response);
...
var HotColdSurveyOptions = function($el, response) {

  this.chart = {
    type: "bar"
  };
  this.title = {
    text: subWithResponse($el.data('title'), response)
  };
  this.subtitle = {
    text: subWithResponse($el.data('subtitle'), response)
  };
  this.yAxis = {
  ...
  this.series = [{
      ...
      name: $el.data('cold-label'),
      data: allSeries.cold
    }];

  this.xAxis = [{
      categories: allSeries.labels,
      reversed: true,
      labels: {
        step: 1
      }
  ...
  this.tooltip = {
    formatter: function() {
      return "<b>\"" + this.point.category + "\"</b> " + this.series.name +": <b>" + Highcharts.numberFormat(Math.abs(this.point.y), 0) + "</b>";
    }
  }
};
...
var hotColdSurveySeries = function(response) {
  ...
  //
  // granularity: null
  // lookup_keys: [keywords, _id]
  // range: null
  // results:
  //   0:
  //     _id: {$oid:5465ab4a6bac9325e5ab673e}
  //     keywords:
  //       _all:
  //         blocks: 0
  //         favorites: 3
  ...
  for (key in results) {
    ...
    var hot = results[key][hotKey], cold = results[key][coldKey];
    series.labels.push(key);
    series.hot.push(hot * -1);
    series.cold.push(cold);
    if (series.max < hot) { series.max = hot; }
    if (series.max < cold) { series.max = cold; }
  }
...
window.refreshAnalytics = function() {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      } else if (chartType === 'hot-cold-survey') {
        $this.highcharts( new HotColdSurveyOptions($this, response) );
      }
      $refresh.html('Refresh').removeClass('analytics-loading');
    }
  });
